...
#####################################
# Load okR autograder
source("setup/autograder-setup/hw5_iteration/hw5_iteration.ok.R")
AutograderInit()
...
#------------------------------------
calculate_inc_rare=function(prev,d){
  id = prev / d
  return(id)
}
...
prevalence_tr = prevalence_tr %>%
  mutate(incidence_density = calculate_inc_rare(prev = prevalence, d = 5))
...
prevalence_tr = prevalence_tr %>%
  mutate(inc_diff = incidence_density - control_inc)
...
#------------------------------------
inc_diff_loop = vector(length=6)
for(i in 1:length(inc_diff_loop)){
  inc_diff_loop[i] = prevalence_tr$incidence_density[i+1] - control_inc
}
...
symptoms = vector(length = ncol(symptoms_df))
for(i in 1:length(symptoms)){
  symptoms[i] = mean(symptoms_df[[i]], na.rm=TRUE)
}
