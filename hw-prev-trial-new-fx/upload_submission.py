...
# COURSE = "51507" ##
# ASSIGNMENT = "224157" ### FILL ME IN
# TOKEN = "zah6y_fc4_l_uaW_s3yU1A"
# PDF_NAME = "hw_upload.pdf"
...
def upload_submission(course_id, assignment_id, student_email, filename, replace, gs_token):


	url = "https://www.gradescope.com/api/v1/courses/" + \
		course_id + "/assignments/" + assignment_id + "/submissions"
	s = requests.Session()
	form_data = {"owner_email": student_email}
	files = {'pdf_attachment': open(filename, 'rb')}
	request_headers = {'access-token': gs_token}
...
if __name__ == '__main__':
	...
	parser.add_argument("course_id", help="course id (find this on gradescope)")
	parser.add_argument("assignment_id", help="assignment id (find this on gradescope)")
	parser.add_argument("token", help="gradescope API token")
	parser.add_argument("pdf_name", help="PDF to upload")
	parser.add_argument('--replace', action='store_true')
	args = parser.parse_args()
	replace = vars(args)['replace']
	email = info['email']
	upload_submission(args.course_id, args.assignment_id, email, args.pdf_name, replace, args.token)
